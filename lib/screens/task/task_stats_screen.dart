...
class _TaskStatsScreenState extends State<TaskStatsScreen> {
  ...
  String _generateInsights(List<Task> tasks, DateTime startOfWeek, DateTime endOfWeek) {
    ...
    for (var t in tasks) {
      if (t.createdAt.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) && t.createdAt.isBefore(endOfWeek)) {
        createdThisWeek++;
      }
      if (t.isCompleted && t.completedAt != null && 
          t.completedAt!.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) && t.completedAt!.isBefore(endOfWeek)) {
        completedThisWeek++;
      }
    }
    ...
    if (completedThisWeek == 0 && createdThisWeek == 0) {
      return isCurrentRealWeek ? "A quiet week so far. Time to plan ahead!" : "No activity recorded during this week.";
  ...
  List<List<int>> _getVelocityForWeek(List<Task> tasks, DateTime startOfWeek, DateTime endOfWeek) {
    List<List<int>> dailyCounts = List.generate(7, (_) => [0, 0]);

    for (var task in tasks) {
      if (task.createdAt.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) && task.createdAt.isBefore(endOfWeek)) {
        dailyCounts[task.createdAt.weekday - 1][0]++; 
      }
      if (task.isCompleted && task.completedAt != null) {
        if (task.completedAt!.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) && task.completedAt!.isBefore(endOfWeek)) {
  ...
  List<int> _getTasksCompletedForWeek(List<Task> tasks, DateTime startOfWeek, DateTime endOfWeek) {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: isCurrentRealWeek ? null : () => _changeWeek(7),
                  ),
                ],
              ),
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.auto_awesome, size: 32, color: Colors.amber),
              ...
              const Text("Task Velocity", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              Row(
                children: [
                  Icon(Icons.circle, size: 12, color: Colors.orange.shade300),
                  const SizedBox(width: 4),
                  const Text("Created", style: TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(width: 16),
              ...
              SizedBox(
                height: 200,
                child: _buildVelocityChart(context, tasks, startOfWeek, endOfWeek, isCurrentRealWeek),
              ),
              const SizedBox(height: 40),

              // 3. PRODUCTIVITY BAR CHART
              const Text("Productivity", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              const Text("Total tasks finished per day", style: TextStyle(fontSize: 12, color: Colors.grey)),
              const SizedBox(height: 16),
              SizedBox(
                height: 200,
                child: _buildProductivityChart(context, tasks, startOfWeek, endOfWeek, isCurrentRealWeek),
              ),
              const SizedBox(height: 40),

              // 4. CATEGORY PIE CHART (ALL TIME)
              const Text("All-Time Distribution", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              SizedBox(
                height: 200,
                child: _buildPieChart(context, tasks),
              ),
              const SizedBox(height: 40),
            ],
          ),
  ...
  Widget _buildVelocityChart(BuildContext context, List<Task> tasks, DateTime startOfWeek, DateTime endOfWeek, bool isCurrentRealWeek) {
  ...
  Widget _buildProductivityChart(BuildContext context, List<Task> tasks, DateTime startOfWeek, DateTime endOfWeek, bool isCurrentRealWeek) {
    final dailyCounts = _getTasksCompletedForWeek(tasks, startOfWeek, endOfWeek); 
    final todayIndex = DateTime.now().weekday - 1; 
    final weekdays = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  ...
  Widget _buildPieChart(BuildContext context, List<Task> tasks) {
    ...
    if (categoryData.isEmpty) return const Center(child: Text("No categories used."));
