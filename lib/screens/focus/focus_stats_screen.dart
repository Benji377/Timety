...
class _FocusStatsScreenState extends State<FocusStatsScreen> {
  ...
  // --- DATA PROCESSORS ---
  List<int> _getFocusMinutesForWeek(List<FocusSession> sessions, DateTime startOfWeek, DateTime endOfWeek) {
    List<int> dailyMins = List.filled(7, 0);
    for (var s in sessions) {
      if (s.startTime.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) && s.startTime.isBefore(endOfWeek)) {
  ...
  Map<String, int> _getTagData(List<FocusSession> sessions, List<FocusTag> tags) {
    Map<String, int> data = {};
    for (var s in sessions) {
  ...
  Widget build(BuildContext context) {
    ...
    final clockSessions = sessions.where((s) => _isSameDay(s.startTime, _selectedDayForClock)).toList();
    int clockTotalMins = clockSessions.fold(0, (sum, s) => sum + (s.totalSecondsFocused ~/ 60));

    return Scaffold(
      body: sessions.isEmpty 
        ? const Center(child: Text("Complete some focus sessions to see data!"))
        : ListView(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Column(
                    children: [
                      Text(isCurrentRealWeek ? "This Week" : "Past Week", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      Text(weekRangeLabel, style: const TextStyle(color: Colors.grey, fontSize: 12)),
                    ],
                  ),
              ...
              const Text("Daily Focus Rhythm", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(color: isSelected ? Colors.white : Colors.grey.shade700, fontWeight: FontWeight.bold),
                        ),
                      ),
                    );
                  }),
              ...
              Center(
                child: SizedBox(
                  ...
                  height: 250,
                  child: CustomPaint(
                    painter: _ClockPainter(sessions: clockSessions, color: Theme.of(context).colorScheme.primary),
                    child: Center(
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Text("${clockTotalMins ~/ 60}h ${clockTotalMins % 60}m", style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900)),
                          Text("${clockSessions.length} sessions", style: const TextStyle(color: Colors.grey)),
                        ],
                      ),
                    ),
                  ),
              ...
              // --- WEEKLY BAR CHART ---
              const Text("Weekly Focus Volume", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Text("Total focus minutes per day", style: TextStyle(fontSize: 12, color: Colors.grey)),
              const SizedBox(height: 16),
              SizedBox(
                height: 200,
                child: _buildBarChart(context, sessions, startOfWeek, endOfWeek, isCurrentRealWeek),
              ),
              const SizedBox(height: 40),

              // --- TAG BREAKDOWN (PIE CHART) ---
              const Text("Time by Tags", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              SizedBox(
                height: 200,
  ...
  Widget _buildBarChart(BuildContext context, List<FocusSession> sessions, DateTime startOfWeek, DateTime endOfWeek, bool isCurrentRealWeek) {
        ...
        barTouchData: BarTouchData(
          ...
          touchTooltipData: BarTouchTooltipData(
            getTooltipItem: (group, groupIndex, rod, rodIndex) {
              return BarTooltipItem("${rod.toY.toInt()} min", const TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
            }
          ),
        ),
            ...
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (double value, TitleMeta meta) {
                ...
                return Padding(
                  ...
                  child: Text(weekdays[dayIndex], style: TextStyle(fontSize: 12, fontWeight: isToday ? FontWeight.bold : FontWeight.normal, color: isToday ? Theme.of(context).colorScheme.primary : Colors.grey)),
                );
              },
            ),
          ),
          ...
          return BarChartGroupData(
            ...
            barRods: [
              BarChartRodData(
                ...
                color: isToday ? Theme.of(context).colorScheme.primary : Colors.blue.shade200, 
                width: 16,
  ...
  Widget _buildTagPieChart(List<FocusSession> sessions, List<FocusTag> tags) {
    ...
    if (tagData.isEmpty || tagData.values.every((v) => v == 0)) return const Center(child: Text("No tagged focus time."));
...
class _ClockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawMarker(String text, double angle) {
      textPainter.text = TextSpan(text: text, style: const TextStyle(color: Colors.grey, fontSize: 10));
      textPainter.layout();
    ...
    for (var session in sessions) {
      ...
      double startHour = session.startTime.hour + (session.startTime.minute / 60.0);
      ...
      if (!session.startTime.isAtSameMomentAs(end) && end.day != session.startTime.day) {
        endHour = 24.0;
      }
