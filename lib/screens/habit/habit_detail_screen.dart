...
class _HabitDetailScreenState extends State<HabitDetailScreen> {
  ...
  Color _selectedColor = AppTheme.infoColor;
  ...
  final List<Color> _colorOptions = [
    AppTheme.infoColor, AppTheme.successColor, AppTheme.warningColor, 
    AppTheme.errorColor, Colors.purple, Colors.teal
  ];
  ...
  void _saveHabit() {
    ...
    final newHabit = Habit(
      ...
      targetDaysPerWeek: _frequency == HabitFrequency.weeklyFlexible ? _targetDaysPerWeek : null,
      targetWeekdays: _frequency == HabitFrequency.weeklyExact ? _selectedWeekdays.toList() : null,
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            // --- COLOR PICKER ---
            const Text("Habit Color", style: TextStyle(fontWeight: AppTheme.fwBold)),
            ...
            // --- FREQUENCY ---
            const Text("Frequency", style: TextStyle(fontWeight: AppTheme.fwBold)),
            const SizedBox(height: AppTheme.spaceSmall),
            SegmentedButton<HabitFrequency>(
              segments: const [
                ButtonSegment(value: HabitFrequency.daily, label: Text("Daily")),
                ButtonSegment(value: HabitFrequency.weeklyFlexible, label: Text("Flexible")),
                ButtonSegment(value: HabitFrequency.weeklyExact, label: Text("Specific")),
              ],
              selected: {_frequency},
            ...
            if (_frequency == HabitFrequency.weeklyFlexible) ...[
              Card(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      Text("Goal: $_targetDaysPerWeek days a week", style: const TextStyle(fontWeight: AppTheme.fwBold)),
                      Slider(
                        value: _targetDaysPerWeek.toDouble(),
                        min: 1, max: 7, divisions: 6,
                        activeColor: _selectedColor,
                        onChanged: (val) => setState(() => _targetDaysPerWeek = val.toInt()),
                      ),
                    ],
                  ),
                ),
            ...
            // --- TIME REMINDER ---
            const Text("Reminder Time (Optional)", style: TextStyle(fontWeight: AppTheme.fwBold)),
            const SizedBox(height: AppTheme.spaceSmall),
            Card(
              child: ListTile(
                leading: Icon(Icons.notifications_active, color: _selectedColor),
                title: Text(_targetTime != null ? _targetTime!.format(context) : "No specific time"),
                trailing: _targetTime != null 
                    ? IconButton(icon: const Icon(Icons.clear), onPressed: () => setState(() => _targetTime = null))
                    : const Icon(Icons.edit),
                onTap: () async {
                  final time = await showTimePicker(context: context, initialTime: _targetTime ?? const TimeOfDay(hour: 8, minute: 0));
