...
class _HabitStatsScreenState extends State<HabitStatsScreen> {
  ...
  int _calculateBestStreak(Habit habit) {
    ...
    Set<String> activeDates = habit.completions.map((dt) => 
  ...
  int _calculateCurrentStreak(Habit habit) {
    ...
    if (activeDates.contains(formatDate(checkDate))) {
      ...
      checkDate = checkDate.subtract(const Duration(days: 1)); // Done yesterday, grace period
    } else {
  ...
  List<int> _getCompletionsForWeek(List<Habit> habits, DateTime startOfWeek, DateTime endOfWeek) {
    List<int> dailyCounts = List.filled(7, 0);
    for (var habit in habits) {
      for (var c in habit.completions) {
        if (c.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) && c.isBefore(endOfWeek)) {
  ...
  Widget build(BuildContext context) {
    ...
    String weekRangeLabel = "${DateFormat('MMM d').format(startOfWeek)} - ${DateFormat('MMM d, yyyy').format(endOfWeek)}";
    bool isCurrentRealWeek = DateTime.now().isAfter(startOfWeek) && DateTime.now().isBefore(endOfWeek);

    // KPI Calculations
    int totalCompletions = habits.fold(0, (sum, h) => sum + h.completions.length);
    ...
    return Scaffold(
      body: habits.isEmpty 
        ? const Center(child: Text("Create and complete habits to unlock insights!"))
        : ListView(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Column(
                    children: [
                      Text(isCurrentRealWeek ? "This Week" : "Past Week", style: const TextStyle(fontWeight: AppTheme.fwBold, fontSize: AppTheme.fsBodyLarge)),
                      Text(weekRangeLabel, style: const TextStyle(color: Colors.grey, fontSize: AppTheme.fsBodySmall)),
                    ],
                  ),
                  IconButton(icon: const Icon(Icons.chevron_right), onPressed: isCurrentRealWeek ? null : () => _changeWeek(7)),
                ],
              ),
              ...
              Wrap(
                ...
                children: [
                  _buildStatCard("Total Logged", "$totalCompletions", Icons.timeline, AppTheme.infoColor),
                  _buildStatCard("Active Habits", "${habits.length}", Icons.all_inclusive, AppTheme.successColor),
                  _buildStatCard("Best Streak", "$allTimeBestStreak 🔥", Icons.military_tech, AppTheme.warningColor),
                ],
              ),
              const SizedBox(height: AppTheme.space3XLarge),

              // --- WEEKLY VOLUME CHART ---
              const Text("Weekly Velocity", style: TextStyle(fontSize: AppTheme.fsHeadingSmall, fontWeight: AppTheme.fwBold)),
              const Text("Total habit completions per day", style: TextStyle(fontSize: AppTheme.fsBodySmall, color: Colors.grey)),
              const SizedBox(height: AppTheme.spaceLarge),
              SizedBox(
                height: 200,
                child: _buildBarChart(habits, startOfWeek, endOfWeek, isCurrentRealWeek),
              ),
              const SizedBox(height: AppTheme.space3XLarge),

              // --- TIME OF DAY PIE CHART ---
              const Text("When Do You Build Habits?", style: TextStyle(fontSize: AppTheme.fsHeadingSmall, fontWeight: AppTheme.fwBold)),
              const Text("Completion distribution by time of day", style: TextStyle(fontSize: AppTheme.fsBodySmall, color: Colors.grey)),
              const SizedBox(height: AppTheme.spaceLarge),
              SizedBox(
                height: 220,
                child: _buildTimePieChart(habits),
              ),
              const SizedBox(height: AppTheme.space3XLarge),

              // --- LEADERBOARD ---
              const Text("Current Streaks", style: TextStyle(fontSize: AppTheme.fsHeadingSmall, fontWeight: AppTheme.fwBold)),
              ...
              ...habits.map((habit) {
                ...
                return Card(
                  ...
                  shape: RoundedRectangleBorder(
                    side: BorderSide(color: Color(habit.colorValue).withValues(alpha: AppTheme.opacityLight), width: 1),
                    borderRadius: AppTheme.brMedium,
                  ),
                  child: ListTile(
                    leading: Icon(Icons.stars, color: Color(habit.colorValue)),
                    title: Text(habit.name, style: const TextStyle(fontWeight: AppTheme.fwBold)),
                    trailing: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
                        Text("$currentStreak 🔥", style: const TextStyle(fontSize: 16, fontWeight: AppTheme.fwExtraBold, color: AppTheme.warningColor)),
                        Text("Best: $bestStreak", style: const TextStyle(fontSize: 10, color: Colors.grey)),
                      ],
                    ),
                  ),
                );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(title, textAlign: TextAlign.center, style: TextStyle(fontSize: AppTheme.fsCaption, color: Colors.grey.shade700, fontWeight: AppTheme.fwBold)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildBarChart(List<Habit> habits, DateTime startOfWeek, DateTime endOfWeek, bool isCurrentRealWeek) {
          ...
          return BarChartGroupData(
            ...
            barRods: [
              BarChartRodData(
                ...
                color: isToday ? AppTheme.infoColor : AppTheme.infoColor.withValues(alpha: AppTheme.opacityMedium), 
                width: 16,
  ...
  Widget _buildTimePieChart(List<Habit> habits) {
    ...
    if (data.values.every((v) => v == 0)) return const Center(child: Text("No completion data yet."));
      ...
      if (value > 0) {
        sections.add(
          PieChartSectionData(
            ...
            title: '$key\n($value)',
            radius: 70,
            titleStyle: const TextStyle(fontSize: 10, fontWeight: AppTheme.fwBold, color: Colors.white),
          )
        );
      }
    });
