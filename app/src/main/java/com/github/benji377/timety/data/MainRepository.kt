...
class MainRepository(private val database: AppDatabase) {
    ...
    suspend fun updateTaskStatus(taskId: Int, status: TaskStatus) = database.taskDao().updateTaskStatus(taskId, status)
    ...
    fun getSessionsForDay(startOfDay: Long, endOfDay: Long) = database.focusSessionDao().getSessionsForDay(startOfDay, endOfDay)
    ...
    suspend fun insertFocusMode(focusMode: FocusMode) = database.focusModeDao().insertFocusMode(focusMode)
    suspend fun updateFocusMode(focusMode: FocusMode) = database.focusModeDao().updateFocusMode(focusMode)
    suspend fun deleteFocusMode(focusMode: FocusMode) = database.focusModeDao().deleteFocusMode(focusMode)

    // Daily Events
    fun getEventsForDay(startOfDay: Long, endOfDay: Long) = database.dailyEventDao().getEventsForDay(startOfDay, endOfDay)
