...
class SettingsRepository(private val dataStore: DataStore<Preferences>) {
    ...
    val autoCompleteFocusFlow: Flow<Boolean> = dataStore.data.map { it[AUTO_COMPLETE_FOCUS] ?: false }
    ...
    val dailyMotivationTimeFlow: Flow<String> = dataStore.data.map { it[DAILY_MOTIVATION_TIME] ?: "08:00" }
    val endOfDayCheckupTimeFlow: Flow<String> = dataStore.data.map { it[END_OF_DAY_CHECKUP_TIME] ?: "20:00" }
    val locationApiEndpointFlow: Flow<String> = dataStore.data.map { it[LOCATION_API_ENDPOINT] ?: "https://photon.komoot.io/api/" }
    ...
    suspend fun saveThemePref(theme: String) { dataStore.edit { it[THEME_PREF] = theme } }
    suspend fun saveUse24HourFormat(use24Hour: Boolean) { dataStore.edit { it[USE_24_HOUR_FORMAT] = use24Hour } }
    suspend fun saveDateFormat(format: String) { dataStore.edit { it[DATE_FORMAT] = format } }
    suspend fun saveAutoCompleteFocus(autoComplete: Boolean) { dataStore.edit { it[AUTO_COMPLETE_FOCUS] = autoComplete } }
    suspend fun saveDailyGoalMins(mins: Int) { dataStore.edit { it[DAILY_GOAL_MINS] = mins } }
    suspend fun saveMaxStopwatchMins(mins: Int) { dataStore.edit { it[MAX_STOPWATCH_MINS] = mins } }
    suspend fun saveMaxNodeMins(mins: Int) { dataStore.edit { it[MAX_NODE_MINS] = mins } }
    suspend fun saveUpcomingTasksHorizon(days: Int) { dataStore.edit { it[UPCOMING_TASKS_HORIZON] = days } }
    suspend fun saveDailyMotivationTime(time: String) { dataStore.edit { it[DAILY_MOTIVATION_TIME] = time } }
    suspend fun saveEndOfDayCheckupTime(time: String) { dataStore.edit { it[END_OF_DAY_CHECKUP_TIME] = time } }
    suspend fun saveLocationApiEndpoint(endpoint: String) { dataStore.edit { it[LOCATION_API_ENDPOINT] = endpoint } }
    suspend fun saveAppLocaleCode(code: String) { dataStore.edit { it[APP_LOCALE_CODE] = code } }
