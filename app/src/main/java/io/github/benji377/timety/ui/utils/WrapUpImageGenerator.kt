...
object WrapUpImageGenerator {
    ...
    fun generate(
        ...
        y = drawHeader(canvas, name, level, levelTitle, y)
        drawStats(canvas, streak, tasksCompleted, focusMins, habitsMet, y)
        drawFooter(canvas)
    ...
    private fun drawLogo(context: Context, canvas: Canvas, startY: Float): Float {
        ...
        if (drawable != null) {
            drawable.setBounds(PAD.toInt(), startY.toInt(), (PAD + size).toInt(), (startY + size).toInt())
            drawable.draw(canvas)
        }
        return startY + size + 36f
    }

    private fun drawHeader(canvas: Canvas, name: String, level: Int, levelTitle: String, startY: Float): Float {
        var y = startY
        ...
        y = paintTextTopLeft(canvas, "YOUR WRAP-UP", PAD, y, paint)
        y += 52f
        ...
        y = paintFittedText(canvas, "$name's Stats", PAD, y, paint, W - PAD * 2)
        y += 14f
        ...
        y = paintTextTopLeft(canvas, "Level $level • $levelTitle", PAD, y, paint)
        y += 58f

        return y
    }

    private fun drawStats(canvas: Canvas, streak: Int, tasks: Int, focus: Int, habits: Int, headerBottom: Float) {
        val startY = headerBottom + 100f
        var y = startY

        val rows = listOf(
            StatRow("🔥", WARNING_COLOR, "$streak Days", "Current Streak"),
            StatRow("✅", TASK_COLOR, "$tasks", "Tasks Completed"),
            StatRow("⏱️", SUCCESS_COLOR, "$focus", "Focus Minutes"),
            StatRow("🔁", HABIT_COLOR, "$habits", "Habits Met")
        )
    ...
    private fun drawFooter(canvas: Canvas) {
        ...
        canvas.drawText("Generated by Timety", W / 2f, y, paint)
    }

    private fun paintTextTopLeft(canvas: Canvas, text: String, x: Float, y: Float, paint: Paint): Float {
        val baselineY = y - paint.fontMetrics.ascent
        canvas.drawText(text, x, baselineY, paint)
        return y + (paint.fontMetrics.bottom - paint.fontMetrics.top)
    }

    private fun paintFittedText(canvas: Canvas, text: String, x: Float, y: Float, paint: Paint, maxWidth: Float): Float {
    ...
    private data class StatRow(val icon: String, val color: Int, val value: String, val label: String)
}
