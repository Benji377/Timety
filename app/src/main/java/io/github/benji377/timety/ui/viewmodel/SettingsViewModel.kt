...
class SettingsViewModel(
    ...
    val themePref = repository.themePrefFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), "System Default")
    val use24HourFormat = repository.use24HourFormatFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), true)
    val dateFormat = repository.dateFormatFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), "System")
    val autoCompleteFocus = repository.autoCompleteFocusFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), false)
    val dailyGoalMins = repository.dailyGoalMinsFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 90)
    val maxStopwatchMins = repository.maxStopwatchMinsFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 120)
    val maxNodeMins = repository.maxNodeMinsFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 240)
    val upcomingTasksHorizon = repository.upcomingTasksHorizonFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), 7)
    val dailyMotivationTime = repository.dailyMotivationTimeFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), "08:00")
    val endOfDayCheckupTime = repository.endOfDayCheckupTimeFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), "20:00")
    val locationApiEndpoint = repository.locationApiEndpointFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), "https://photon.komoot.io/api/")
    val appLocaleCode = repository.appLocaleCodeFlow.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), "system")
    ...
    fun setAutoCompleteFocus(autoComplete: Boolean) = viewModelScope.launch { repository.saveAutoCompleteFocus(autoComplete) }
    fun setDailyGoalMins(mins: Int) = viewModelScope.launch { repository.saveDailyGoalMins(mins) }
    fun setMaxStopwatchMins(mins: Int) = viewModelScope.launch { repository.saveMaxStopwatchMins(mins) }
    fun setMaxNodeMins(mins: Int) = viewModelScope.launch { repository.saveMaxNodeMins(mins) }
    fun setUpcomingTasksHorizon(days: Int) = viewModelScope.launch { repository.saveUpcomingTasksHorizon(days) }
    ...
    fun setLocationApiEndpoint(endpoint: String) = viewModelScope.launch { repository.saveLocationApiEndpoint(endpoint) }
